...
#    }
    ...
    foreach ($Name in $VMName)
    {
        if ($Name -eq $Pullserver)
        {
            $AllFilesToCopy = $FilesToCopy + $FilesToCopyPullServer
        }
        else
        {
        ...
        xVhdFile "VHDPrep$Name"
        {
            ...
            FileDirectory = $AllFilesToCopy | % {
                if ($_.SourcePath -like "*unattend*")
                {
                    MSFT_xFileDirectory {
                        SourcePath = $_.SourcePath -replace "unattend.xml","$($Name).xml"
